...
public class RotationsFile implements FileSupport<List<Point3D>> {
     ...
     */
    private static final String FILE_NAME = "r";
    /**
     * The name of the file containing the data
     */
    private static final String FILE_EXTENSION = "cyfr";

    /**
     * Constructor which actually creates a new {@link RotationsFile} in the persistence layer.
     *
     * @param measurement The {@link Measurement} to which this file is part of.
     */
    public RotationsFile(@NonNull final Measurement measurement) {
        this.file = measurement.createFile(FILE_NAME, FILE_EXTENSION);
        this.measurement = measurement;
    }

    /**
     * Constructor to reference existing {@link RotationsFile}.
     *
     * @param measurement The {@link Measurement} to which this file is part of.
     * @param file The already existing file which represents the {@link RotationsFile}
     */
    private RotationsFile(@NonNull final Measurement measurement, @NonNull final File file) {
        this.file = file;
        this.measurement = measurement;
    }
    ...
    /**
     * For testing this method helps to load the stored data from a {@link RotationsFile}.
     *
     * @return the {@link Point3D} data restored from the {@code RotationsFile}
    ...
    public List<Point3D> deserialize() throws FileCorruptedException {
        ...
        final int pointCount = measurement.getMetaFile().deserialize().getPointMetaData().getCountOfRotations();
        return MeasurementSerializer.deserializePoint3dData(bytes, pointCount);
    }

    /**
     * Loads an existing {@link RotationsFile} for a specified {@link Measurement}.
     *
     * @return the {@link RotationsFile} link to the file
     * @throws IllegalStateException if there is no such file
     */
    public static RotationsFile loadFile(@NonNull final Measurement measurement) {
        final File file = FileUtils.generateMeasurementFilePath(measurement, FILE_NAME, FILE_EXTENSION);
        Validate.isTrue(file.exists());
        return new RotationsFile(measurement, file);
    }
}
